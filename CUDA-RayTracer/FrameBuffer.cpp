...
int FrameBuffer::attach(GLenum attachment_type, GLint internal_format, GLenum format, GLenum type) 
{
    int success = 0;
...
int FrameBuffer::attach_rbo(GLenum renderbuffertarget, GLenum internal_format)
{
...
unsigned int FrameBuffer::get_color_tex_id(int i)
{
...
void FrameBuffer::set_attachment_size(GLsizei width, GLsizei height)
{
    (this)->width = width;
    (this)->height = height;
...
unsigned int FrameBuffer::get_depth_tex_id()
{
    if (nmb_depth_attachemnts != 1) {
...
unsigned int FrameBuffer::get_stencil_tex_id()
{
    if (nmb_stencil_attachemnts != 1) {
        std::cout << "Error: the depth attachment is NULL." << std::endl;
        return 0;
...
std::shared_ptr<Attachment> FrameBuffer::get_color_attachment(int i)
{
    if (i >= nmb_color_attachemnts) {
        std::cout << "Error: color attachment " << i << " is NULL." << std::endl;
        return nullptr;
...
std::shared_ptr<Attachment> FrameBuffer::get_depth_attachment()
{
    if (nmb_depth_attachemnts != 1) {
        std::cout << "Error: the depth attachment is NULL." << std::endl;
        return nullptr;
...
std::shared_ptr<Attachment> FrameBuffer::get_stencil_attachment()
{
    if (nmb_stencil_attachemnts != 1) {
