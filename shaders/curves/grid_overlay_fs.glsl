...
void main() {
    ...
    vec3 wo = normalize(camPos - grid_pos);
    ...
    float depth_1 = perspective_mode ? 0.99999f : compute_depth(grid_pos);

    vec4 wireframe = texture(wireframe_tex, tex_coords_remap);

    float grid_alpha = perspective_mode ? grid_value.a : grid_value.a * (t > 0.f ? 1.f : 0.f) * fading;
    
    if (depth_0 > depth_1 && grid_alpha > EPSILON_ALPHA) {
        vec3 color = mix(wireframe.rgb, grid_value.rgb, grid_alpha);
        //float alpha = mix(wireframe.a, 1.f, grid_alpha) * pow(1.f - max(0.f, dot(vec3(0.f, 1.f, 0.f), wo)), 5.f);//max(0.0, dot(vec3(0, 1, 0), wo));

        vec3 n = camPos.y > 0.f ? vec3(0.f, 1.f, 0.f) : vec3(0.f, -1.f, 0.f);
        float alpha = (1.f - pow(1.f - max(0.f, dot(n, wo)), 5.f)) * mix(wireframe.a, 1.f, grid_alpha);
