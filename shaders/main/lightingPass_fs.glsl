...
void main()
{
    vec2 tex_coords_remap = remap(TexCoords, vec2(0.f), vec2(1.f), vec2(0.f), vec2(tex_dim / scr_dim));

    vec3 FragPos = texture(gPosition, tex_coords_remap).rgb;
    vec3 n = texture(gNormal, tex_coords_remap).rgb;
    vec3 a = texture(gAlbedo, tex_coords_remap).rgb;
    vec3 emissive = texture(gEmissive, tex_coords_remap).rgb;
    float ao = texture(gMetallicRoughAO, tex_coords_remap).r;
    float r = texture(gMetallicRoughAO, tex_coords_remap).g;
    float m = texture(gMetallicRoughAO, tex_coords_remap).b;
    ...
    float shadow = 0.f;// texture(gShadows, TexCoords).r;
